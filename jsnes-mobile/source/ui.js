    ...
    (function ($) {
        $.fn.JSNESUI = function (roms) {
            ...
            var UI = function (nes) {
                ...
                //$('<input type="button" value="bbs" class="nes-bbs" onclick="location.href=\'#'">').appendTo(self.controls)
...
//                        });
                ...
                if ($.offset) {
                    self.screen.mousedown(function (e) {
                        if (self.nes.mmap) {
                            self.nes.mmap.zapperFired = true;
                            // FIXME: does not take into account zoom
                            self.nes.mmap.zapperX = e.pageX - self.screen.offset().left;
                            self.nes.mmap.zapperY = e.pageY - self.screen.offset().top;
                        }
                    }).mouseup(function () {
                        setTimeout(function () {
                            if (self.nes.mmap) {
                                self.nes.mmap.zapperFired = false;
                                self.nes.mmap.zapperX = 0;
                                self.nes.mmap.zapperY = 0;
                            }
                        }, 500);
                    });
                }
